...
public:
    ...
    int go_back(json_graph, use_planner = true, bring_back = true){
        ...
        std::vector<pair<int,int>> drone_indices(num_cf); // odom_linear coordinates -> indices
        std::vector<std::vector<bool>> adjacency_mat(num_cf+1, std::vector<bool>(num_cf+1, false));
        for(int i = 1; i <= num_cf; i++){
            for(int j = 1; j <= num_cf; j++){
                if(json_graph[drone_indices[i].first][drone_indices[i].second][drone_indices[j].first][drone_indices[j].second]){
                    adjacency_mat[i][j] = true; // inLoS
                    adjacency_mat[j][i] = true;
                }
            }
        }

        for(int i = 1; i <= num_cf; i++){
            if(json_graph[0][0][drone_indices[i].first][drone_indices[i].second]){
                adjacency_mat[0][i] = true;
                adjacency_mat[i][0] = true;
            }
        }
