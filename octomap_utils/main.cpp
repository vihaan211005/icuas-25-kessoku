...
std::vector<Circle> findCirclesOnSlice(const std::vector<std::vector<std::vector<uint8_t>>>& binary_slice,
                                       double resolution, double min_x, double min_y,
                                       double min_z, double max_z, const octomap::OcTree& tree) {
    ...
    for (size_t y = 0; y < size_y; ++y) {
        for (size_t x = 0; x < size_x; ++x) {
            if (binary_slice[1][y][x] == 1 && !visited[y][x]) {
                std::vector<std::pair<int, int>> component;
                bfs(x, y, binary_slice[1], visited, component);
...
int main(int argc, char** argv) {
    ...
    std::vector<Circle> circles = findCirclesOnSlice(binary_slice, resolution, min_x, min_y, min_z, max_z, tree);
